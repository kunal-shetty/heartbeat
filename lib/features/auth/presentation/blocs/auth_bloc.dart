...
class AuthVerifyOtpEvent extends AuthEvent {
  ...
  @override List<Object?> get props => [phone, token];
}
...
class AuthRegisterEvent extends AuthEvent {
  ...
  AuthRegisterEvent({
    ...
    required this.displayName, this.phone, this.email,
  });
  @override List<Object?> get props => [userId, username, displayName];
}
...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onSignInWithPhone(AuthSignInWithPhoneEvent event, Emitter<AuthState> emit) async {
    emit(AuthLoadingState());
    try {
...
Future<void> _onSignUpWithEmail(
  ...
  emit(AuthLoadingState());
  try {
    final user = await signInWithEmail.repository.signUpWithEmail(
      email: event.email,
  ...
  Future<void> _onSignInWithEmail(AuthSignInWithEmailEvent event, Emitter<AuthState> emit) async {
    emit(AuthLoadingState());
    try {
      final user = await signInWithEmail(email: event.email, password: event.password);
      emit(AuthAuthenticatedState(user));
    } on AuthFailure catch (e) {
      emit(AuthErrorState(e.message));
    }
  ...
  Future<void> _onSignInWithGoogle(AuthSignInWithGoogleEvent event, Emitter<AuthState> emit) async {
  emit(AuthLoadingState());
  try {
